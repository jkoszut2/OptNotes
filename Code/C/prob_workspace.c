...
int cone_start_idxs[num_cones] = {0};
int e_socs[num_cone_vars] = {0}; // \textbb{e} vector
...
// "PROCESSED" PROBLEM DATA STARTS BELOW
// idxint n = 3;
// idxint m = 4;
// idxint p = 1;
// idxint l = 1;
...
// idxint* q = (int *) &ecos_q;
// idxint nexc = 0;
// pfloat* Gpr = canon_G_ECOS_x;

// nnzK is num of nonzeros in upper tri of KKT matrix
// Found using below code and #defined in prob_workspace header
// #if STATICREG == 1
// 	idxint nnzK = At->nnz + Gt->nnz + l + n + p;
// #else
// 	idxint nnzK = At->nnz + Gt->nnz + l;
// #endif

// for(int i=0; i<workspace->C->nsoc; i++ ){
//     nnzK += (workspace->C->soc[i].p*(workspace->C->soc[i].p+1))/2;
// }